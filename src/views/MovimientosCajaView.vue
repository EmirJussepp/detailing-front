...
watch(
  admin,
  (isAdm) => {
    turnoSel.value = turnoUI(isAdm ? "MAÑANA" : getTurnoOperativo())
  },
  { immediate: true }
)
...
// =========================
// Caja actual del turno
// =========================
async function refreshCaja() {
  cajaAbierta.value = null
  cajaCheck.value = { ok: false, error: "" }
  ...
  try {
    ...
    if (status === 404) {
      cajaCheck.value = { ok: false, error: "No hay caja ABIERTA para este turno." }
    } else if (status === 403) {
      cajaCheck.value = { ok: false, error: "No tenés permisos para consultar Caja." }
    } else {
      cajaCheck.value = { ok: false, error: msg || "Error consultando caja." }
    }
  }
}
...
async function refreshMovimientos() {
  ...
  if (!cajaAbierta.value?.cajaId) {
    infoMsg.value = "No hay caja abierta para este turno. No podés crear movimientos."
    return
  }
  ...
  try {
    ...
    const montoInicial =
      Number(cajaAbierta.value?.montoInicial ?? cajaAbierta.value?.monto_inicial ?? 0) || 0
    ...
    errorMsg.value =
      ...
      e?.message ||
      "Error cargando movimientos del turno."
  }
}
...
// Permisos operativos
// =========================
const canCreate = computed(() => cajaCheck.value?.ok === true && Boolean(cajaAbierta.value?.cajaId))

const tituloEstado = computed(() => {
  if (canCreate.value) return "Caja ABIERTA"
...
const conceptos = ["GASTO", "RETIRO", "APORTE", "DEVOLUCION", "AJUSTE"]
...
async function crearMovimiento() {
  ...
  try {
    if (!canCreate.value) {
      throw new Error(cajaCheck.value?.error || "No hay caja ABIERTA.")
    }
...
function exportCSV() {
  ...
  a.href = url
  a.download = `movimientos_turno_${turnoBE(turnoSel.value)}.csv`
  a.click()
...
>
  ...
  <option value="TARDE">TARDE</option>
          ...
          <div class="col-12 col-md-9">
            ...
            <div v-else class="small text-secondary">
              ...
              <template v-if="canCreate">
                ...
                <span class="ms-2">
                  Responsable: <b>{{ cajaAbierta?.userId }}</b>
                </span>
                <span class="ms-2">
              ...
              <template v-else>
                <span>No hay caja abierta para este turno.</span>
              </template>
            </div>
          </div>
        </div>
    ...
    <div class="row g-3 mb-4" v-if="movimientos.length || canCreate">
      <div class="col-12 col-md-3">
    ...
    <div class="card bg-panel border-0 shadow-sm mb-4" v-if="canCreate">
      <div class="card-body">
    ...
    <div class="card bg-panel border-0 shadow-sm">
      <div class="card-body">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
          <h2 class="h6 mb-0">Movimientos del turno {{ turnoSel }}</h2>
        ...
        <div v-if="!movimientosFiltrados.length" class="text-secondary small">
          No hay movimientos para mostrar en este turno.
        </div>
